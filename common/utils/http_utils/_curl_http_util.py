...
class CurlHttpUtil(RootHttpAbstract):
    @http_log_decorator()
    ...
    @http_log_decorator()
    def post(self, url: str, headers: dict, data: Optional[Union[str, dict, list, bytes]] = None,
    ...
    def __request(self, method: Literal["GET", "POST", "PUT", "DELETE", "OPTIONS", "HEAD", "TRACE", "PATCH"], url: str,
        ...
        submit_data = "" if not data else json.dumps(data, separators=(',', ':')) if isinstance(data, dict) else data
        try:
