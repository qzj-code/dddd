...
class OrderDetailParser:
    ...
    def parse_order_detail(cls, order_data) -> OrderInfoModel:
        journeys = []
        if not order_data['resbean'].get('ticketList'):
            raise ServiceError(ServiceStateEnum.TICKET_NUMBER_INFO_MISTAKE)
        segment_list = cls.parse_segments(order_data['resbean']['ticketList'][0])
        bundles = cls.parse_bundlel(order_data['resbean'])
        passenger_infos = cls.parse_passenger_info(order_data['resbean'])
        ...
        if len(journeys) > 1:
            ...
            if len(journeys) == 0:
                ...
                journey = journeys[0]
        if order_data['resbean']['orderStatus'] == 3:
        ...
        if not passenger_infos[0].ticket_info_list:
            ...
            issued = True
        pnr = order_data['resbean']['ticketList'][0]['travelerlist'][0]['pnr']
        amount = order_data['resbean']['showOrderPrices']
        ticket_carrier = order_data['resbean']['ticketList'][0]['flightCom']
        return OrderInfoModel.model_validate({
            ...
            'totalPrice': amount,
            'orderNo': order_data['resbean']['orderNumber']
        })
    ...
    def parse_bundlel(cls, resbean):
        ...
        if adt_fare > 0:
            price_info.append(PriceInfoModel.model_validate({
                ...
                "fare": adt_fare / adt_count,
                "tax": adt_tax / adt_count,
                "ext": {
                    "oiFee": adt_fee / adt_count
                }
            }))
        if chd_fare > 0:
            price_info.append(PriceInfoModel.model_validate({
                ...
                "fare": chd_fare / chd_count,
                "tax": chd_tax / chd_count,
                "ext": {
                    "oiFee": chd_fee / chd_count
                }
            }))
